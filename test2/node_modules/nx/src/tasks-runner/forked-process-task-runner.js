    ...
    forkProcessForBatch({ executorName, taskGraph: batchTaskGraph }, fullTaskGraph) {
        return new Promise((res, rej) => {
            try {
                ...
                const printedTasks = new Set();
                p.on('message', (message) => {
                    switch (message.type) {
                        case batch_messages_1.BatchMessageType.CompleteTask: {
                            this.options.lifeCycle.printTaskTerminalOutput(batchTaskGraph.tasks[message.task], message.result.success ? 'success' : 'failure', message.result.terminalOutput);
