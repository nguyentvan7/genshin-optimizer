...
function handleMessage(socket, data) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        if (payload.type === 'PING') {
            yield handleResult(socket, 'PING', () => Promise.resolve({ response: JSON.stringify(true), description: 'ping' }));
        }
        else if (payload.type === 'REQUEST_PROJECT_GRAPH') {
            yield handleResult(socket, 'REQUEST_PROJECT_GRAPH', () => (0, handle_request_project_graph_1.handleRequestProjectGraph)());
        }
        else if (payload.type === 'HASH_TASKS') {
            yield handleResult(socket, 'HASH_TASKS', () => (0, handle_hash_tasks_1.handleHashTasks)(payload));
        }
        else if (payload.type === 'REQUEST_FILE_DATA') {
            yield handleResult(socket, 'REQUEST_FILE_DATA', () => (0, handle_request_file_data_1.handleRequestFileData)());
        }
        else if (payload.type === 'PROCESS_IN_BACKGROUND') {
            yield handleResult(socket, 'PROCESS_IN_BACKGROUND', () => (0, handle_process_in_background_1.handleProcessInBackground)(payload));
        }
        else if (payload.type === 'RECORD_OUTPUTS_HASH') {
            yield handleResult(socket, 'RECORD_OUTPUTS_HASH', () => (0, handle_outputs_tracking_1.handleRecordOutputsHash)(payload));
        }
        else if (payload.type === 'OUTPUTS_HASHES_MATCH') {
            yield handleResult(socket, 'OUTPUTS_HASHES_MATCH', () => (0, handle_outputs_tracking_1.handleOutputsHashesMatch)(payload));
        }
        else if (payload.type === 'REQUEST_SHUTDOWN') {
            yield handleResult(socket, 'REQUEST_SHUTDOWN', () => (0, handle_request_shutdown_1.handleRequestShutdown)(server, numberOfOpenConnections));
        }
...
function handleResult(socket, type, hrFn) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
...
function lockFileHashChanged() {
    const lockHashes = [
        ...
        .map((file) => file_hasher_1.fileHasher.hashFile(file));
                    ...
                    try {
                        ...
                        if (useNativeWatcher()) {
                            if (!(0, shutdown_utils_1.getWatcherInstance)()) {
                                (0, shutdown_utils_1.storeWatcherInstance)(yield (0, watcher_1.watchWorkspace)(server, handleWorkspaceChanges));
                            ...
                            if (!(0, shutdown_utils_1.getOutputWatcherInstance)()) {
                                (0, shutdown_utils_1.storeOutputWatcherInstance)(yield (0, watcher_1.watchOutputFiles)(handleOutputsChanges));
                            }
                        }
                        else {
